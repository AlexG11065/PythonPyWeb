...
class TrainView(View):
    def get(self, request):
        ...
        self.answer1 = Author.objects.filter(self_esteem=max_self_esteem[
            'max_self_esteem'])  # Какие авторы имеют самую высокую уровень самооценки(self_esteem)?
        ...
        self.answer4 = Author.objects.filter(
            gender__contains='ж').count()  # Сколько авторов женского пола зарегистрировано в системе?

        self.answer5 = Author.objects.aggregate(status_rule=Avg('status_rule'))  # TODO Какой процент авторов согласился с правилами при регистрации?
        ...
        self.answer7 = Author.objects.filter(age=max_age[
            'max_age'])  # .values_list('last_name', 'first_name', flat=True)  # TODO Какой автор имеет наибольший возраст?
